...
// Minimal GSAP type for optional runtime use without depending on the package
declare global {
  interface Window {
    gsap?: {
      to: (
        target: Element | Element[] | Record<string, unknown>,
...
const MacOSDock: React.FC<MacOSDockProps> = ({
  ...
  const getResponsiveConfig = useCallback(() => {
    if (typeof window === 'undefined') {
      return { baseIconSize: 64, maxScale: 1.8, effectWidth: 240 };
    }
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (dockRef.current) {
      ...
      const padding = Math.max(12, baseIconSize * 0.18);
  ...
  const handleAppClick = (appId: string, index: number) => {
    if (iconRefs.current[index]) {
      if (typeof window !== 'undefined' && window.gsap) {
        ...
        gsap.to(iconRefs.current[index] as Element, {
          y: bounceHeight,
          duration: 0.2,
          ease: 'power2.out',
          yoyo: true,
  ...
  const padding = Math.max(12, baseIconSize * 0.18);
    ...
    <div 
      ref={dockRef}
      className={`backdrop-blur-md pointer-events-auto ${className}`}
      style={{
          ...
          return (
            <ContextMenu key={app.id}>
              <ContextMenuTrigger asChild>
                ...
                >
                  <button
                    className="flex flex-col items-center justify-end bg-transparent border-none p-0 w-full h-full"
                    type="button"
                    title={app.name}
              ...
              <ContextMenuContent className="w-64">
                ...
                <ContextMenuItem
                  ...
                  disabled={!isOpen}
                >
                  Show {isOpen ? 'Hide' : 'Show'} {app.name}
                </ContextMenuItem>
                ...
                <ContextMenuItem className="cursor-pointer" variant="destructive">
                  Remove from Dock
                </ContextMenuItem>
